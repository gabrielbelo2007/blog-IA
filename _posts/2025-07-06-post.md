...
1.  **Bernardo, um misto de pelo macio e energia inesgotável, disparou pela porta assim que ela se abriu.**
2.  **Com um latido que parecia um convite à aventura, Bernardo abanava o rabo com a força de um furacãozinho.**
3.  **O focinho curioso de Bernardo explorava cada novo cheiro que o vento trazia da floresta próxima.**
4.  **Apesar de seu tamanho imponente, Bernardo se aninhava no sofá com a delicadeza de um filhote, sonhando com biscoitos.**
'''
```

> A possibilidade dele enviar mais de uma opção, ocorre por conta da função *thinking* que por padrão vem ativada nos modelos do *Gemini*, está sendo implementado na biblioteca da da `google-generativeai` um controle dessa função para alguns modelos como o `2.5-flash`.

Agora é possível ver o histórico da última mensagem usando o seguinte laço:

```py
for message in chat_model.history:
...
Model: Aqui estão algumas opções, escolha a que mais gostar ou use como inspiração:

1.  Com uma pata enlameada e um olhar de quem aprontou, Bernardo entrou correndo na sala.
2.  O fiel Bernardo esperava à porta, como sempre, pelo retorno da sua família.
3.  Foi o faro infalível de Bernardo que revelou o segredo escondido no velho sótão.
4.  O ronco suave de Bernardo sob o sofá era a melodia perfeita para um fim de tarde preguiçoso.
5.  A cauda de Bernardo abanava com a força de um furacão, expressando sua alegria inabalável.
'''
```
...
```
1.  Com uma pata enlameada e um olhar de quem aprontou, Severino entrou correndo na sala.
2.  O fiel Severino esperava à porta, como sempre, pelo retorno da sua família.
3.  Foi o faro infalível de Severino que revelou o segredo escondido no velho sótão.
4.  O ronco suave de Severino sob o sofá era a melodia perfeita para um fim de tarde preguiçoso.
5.  A cauda de Severino abanava com a força de um furacão, expressando sua alegria inabalável.
```
